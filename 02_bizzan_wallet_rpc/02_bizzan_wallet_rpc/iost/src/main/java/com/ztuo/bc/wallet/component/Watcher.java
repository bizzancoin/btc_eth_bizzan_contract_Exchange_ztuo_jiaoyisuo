package com.bizzan.wallet.component;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.bizzan.wallet.entity.Coin;
import com.bizzan.wallet.entity.Deposit;
import com.bizzan.wallet.event.DepositEvent;
import com.bizzan.wallet.service.AccountService;
import com.bizzan.wallet.service.WatcherLogService;
import com.bizzan.wallet.util.HttpClientUtil;
...
public class Watcher extends Thread{
    ...
    private List<Deposit> replayBlock(Long startBlockNumber, Long endBlockNumber) {
    	...
    	try {
			...
			String retStr = this.iostHttpsGet(this.blockApi + "v2/third/get_account_transfer", param.toJSONString());
    ...
    public Long getNetwortBlockHeight() {
		try {
			String retStr = this.iostHttpsGet(this.blockApi + "apikey=" + this.apikey + "&module=block&action=get-latest-block");
			if(!StringUtils.isEmpty(retStr)){
        		JSONObject json = JSON.parseObject(retStr);
        		if(json.getIntValue("code") == 0) {
        			JSONObject data = json.getJSONObject("data");
        			JSONObject block = data.getJSONObject("block");
        			Long height = block.getLong("number");
        			return height;
        		}
        	}
        	return 0L;
    ...
    private String iostHttpsGet(String url) throws KeyManagementException, ClientProtocolException, NoSuchAlgorithmException, KeyStoreException, IOException {
    	return HttpClientUtil.doHttpsGet(url, null, null);
    }
}
